...
function createWeightedSearch(types, client) {
  ...
  return function search(queryString) {
    ...
    var constraints = terms.map((term, i) => combinedSearchPaths.map(joinedPath => `${joinedPath} match $t${i}`)).filter(constraint => constraint.length > 0);
    ...
    var selection = selections.length > 0 ? `...select(${selections.join(',\n')})` : '';
    var query = `*[${filters.join('&&')}][0...$limit]{_type, _id, ${selection}}`;
