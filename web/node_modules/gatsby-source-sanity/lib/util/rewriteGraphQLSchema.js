...
function transformAst(ast, context) {
    return Object.assign({}, ast, { definitions: ast.definitions
...
function transformInterfaceTypeDefinition(node, context) {
    const fields = node.fields || [];
    return Object.assign({}, node, { fields: fields.map(field => transformFieldNodeAst(field, node, context)), name: Object.assign({}, node.name, { value: getTypeName(node.name.value) }) });
}
...
function transformFieldNodeAst(node, parent, context) {
    const field = Object.assign({}, node, { name: maybeRewriteFieldName(node, parent, context), type: rewireIdType(makeNullable(node.type)), description: undefined, directives: [] });
...
function maybeRewriteFieldName(field, parent, context) {
    ...
    return Object.assign({}, field.name, { value: newFieldName });
}
