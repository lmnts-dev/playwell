...
const validatePluginOptions = rawPluginOptions => {
  const schema = yup.object().shape({
    repositoryName: yup.string().nullable().required('Invalid Repository Name.'),
    accessToken: yup.string().nullable().required('Invalid access token.'),
    ...
    typePathsFilenamePrefix: yup.string().nullable().required('Invalid typePaths filename prefix.'),
    schemasDigest: yup.string().nullable().required('Invalid Schemas digest.'),
    pathResolver: yup.mixed().nullable().test('is function', '${path} is not a function', value => value === undefined || isFunction(value)),
    shouldNormalizeImage: yup.mixed().notRequired().nullable(),
    lang: yup.string().notRequired().nullable(),
    plugins: yup.array().notRequired().nullable()
  });
