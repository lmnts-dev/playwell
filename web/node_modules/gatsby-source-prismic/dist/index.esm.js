...
const validatePluginOptions = rawPluginOptions => {
  const schema = object().shape({
    repositoryName: string().nullable().required('Invalid Repository Name.'),
    accessToken: string().nullable().required('Invalid access token.'),
    fetchLinks: array().of(string().required()).default([]),
    ...
    typePathsFilenamePrefix: string().nullable().required('Invalid typePaths filename prefix.'),
    schemasDigest: string().nullable().required('Invalid Schemas digest.'),
    pathResolver: mixed().nullable().test('is function', '${path} is not a function', value => value === undefined || isFunction(value)),
    shouldNormalizeImage: mixed().notRequired().nullable(),
    lang: string().notRequired().nullable(),
    plugins: array().notRequired().nullable()
  });
