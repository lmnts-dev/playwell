(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		module.exports = factory(require("path"), require("lodash.template"), require("webpack-sources"), require("memory-fs"), require("webpack"), require("table"), require("chalk"), require("google-oauth2-node"), require("guess-ga"), require("flat-cache"), require("googleapis"));
	else if(typeof define === 'function' && define.amd)
		define(["path", "lodash.template", "webpack-sources", "memory-fs", "webpack", "table", "chalk", "google-oauth2-node", "guess-ga", "flat-cache", "googleapis"], factory);
	else {
		var a = typeof exports === 'object' ? factory(require("path"), require("lodash.template"), require("webpack-sources"), require("memory-fs"), require("webpack"), require("table"), require("chalk"), require("google-oauth2-node"), require("guess-ga"), require("flat-cache"), require("googleapis")) : factory(root["path"], root["lodash.template"], root["webpack-sources"], root["memory-fs"], root["webpack"], root["table"], root["chalk"], root["google-oauth2-node"], root["guess-ga"], root["flat-cache"], root["googleapis"]);
		for(var i in a) (typeof exports === 'object' ? exports : root)[i] = a[i];
	}
})(global, function(__WEBPACK_EXTERNAL_MODULE__0__, __WEBPACK_EXTERNAL_MODULE__3__, __WEBPACK_EXTERNAL_MODULE__4__, __WEBPACK_EXTERNAL_MODULE__5__, __WEBPACK_EXTERNAL_MODULE__6__, __WEBPACK_EXTERNAL_MODULE__12__, __WEBPACK_EXTERNAL_MODULE__13__, __WEBPACK_EXTERNAL_MODULE__16__, __WEBPACK_EXTERNAL_MODULE__17__, __WEBPACK_EXTERNAL_MODULE__18__, __WEBPACK_EXTERNAL_MODULE__19__) {
...
var validateInput = function (routes, graph, debug) {
...
exports.buildMap = function (routes, graph, debug) {
...
exports.getCompilationMapping = function (compilation, entryPoints, debug) {
    var fileChunk = {};
    var mainName = null;
    ...
    compilation
        ...
        if (c.initial) {
            mainName = c.files.filter(function (f) { return f.endsWith('.js'); }).pop();
        }
        if (c.modules && c.modules.length) {
            ...
            if (existingEntries.length > 1) {
                if (debug) {
                    console.warn('There are more than two entry points associated with chunk', c.files[0]);
                }
            }
...
var prefetch_aot_plugin_1 = __webpack_require__(11);
...
var GuessPlugin = /** @class */ (function () {
    ...
    GuessPlugin.prototype.apply = function (compiler) {
        ...
        compiler.hooks.emit.tapAsync({
            ...
            return _this._execute(compilation, cb);
        });
    };
    GuessPlugin.prototype._execute = function (compilation, cb) {
        var _this = this;
        extractRoutes(this._config).then(function (routes) { return __awaiter(_this, void 0, void 0, function () {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        data = _a.sent();
                        return [2 /*return*/, this._executePrefetchPlugin(data, routes, compilation, cb)];
                    case 2:
                        err_1 = _a.sent();
                        console.error(err_1);
                        cb();
    ...
    GuessPlugin.prototype._executePrefetchPlugin = function (data, routes, compilation, cb) {
...
var ConcatSource = __webpack_require__(4).ConcatSource;
var PrefetchPlugin = /** @class */ (function () {
    ...
    PrefetchPlugin.prototype.execute = function (compilation, callback) {
        ...
        var fileChunk = {};
        try {
            var res = utils_1.getCompilationMapping(compilation, new Set(this._config.routes.map(function (r) { return utils_1.stripExtension(r.modulePath); })), this._config.debug);
        ...
        }), this._config.data, !!this._config.debug);
...
var template = __webpack_require__(3);
var ConcatSource = __webpack_require__(4).ConcatSource;
var alterChunk = function (compilation, chunkName, original, toAlter, toBundle) {
    var promise = !toBundle
        ...
        : new Promise(function (resolve, reject) {
            ...
            compiler.run(function (err, stats) {
                ...
                resolve(stats.compilation.assets['./output.js']);
            });
        });
...
var forEachBlock = function (chunk, cb) {
    var blocks = [];
    if (chunk.groupsIterable) {
        blocks = Array.from(chunk.groupsIterable).reduce(function (prev, group) {
            return prev.concat(group.getBlocks().map(function (block) { return ({ chunk: group, block: block }); }));
        }, []);
    }
    else {
        blocks = (chunk.blocks || []).map(function (block) { return ({ chunk: chunk, block: block }); });
    }
    blocks.forEach(cb);
};
var PrefetchAotPlugin = /** @class */ (function () {
    ...
    PrefetchAotPlugin.prototype.execute = function (compilation, callback) {
        var _this = this;
        ...
        this.logger.debug('Route to chunk mapping is', JSON.stringify(routeChunk, null, 2));
        ...
        var generateNeighbors = function (route, currentChunk, c) {
            if (!c.chunk) {
                _this.logger.warn('Cannot find chunk name for', c, 'from route', route);
                return false;
            }
            tableOutput.push([currentChunk, c.chunk, c.probability]);
            return "['" + path_1.join(_this._config.basePath, c.chunk) + "'," + c.probability + "]";
        };
        Object.keys(routeChunk).forEach(function (route) {
            var chunkName = routeChunk[route];
            var currentChunk = compilation.assets[chunkName];
            if (!currentChunk) {
                callback();
                _this.logger.warn("Cannot find the chunk \"" + chunkName + "\" for route \"" + route + "\"");
                return;
            }
            ...
            if (chunkName === mainName) {
                compilationPromises[0] = compilationPromises[0].then(function () {
                    return alterChunk(compilation, chunkName, compilation.assets[chunkName].source(), newCode, false);
                });
            }
            else {
                compilationPromises.push(alterChunk(compilation, chunkName, currentChunk.source(), newCode, false));
            }
        });
        this.logger.info(chalk_1.default.blue('\n\n\n🔮 Guess.js introduced the following prefetching instructions:'));
        console.log(table_1.table(tableOutput));
        Promise.all(compilationPromises)
            .then(function () {
            _this.logger.debug('Chunks altered');
            callback();
        })
            .catch(function (e) {
